...
export function setHeaterById(
    itemId: string,
    updateWith: Record<string, any>
): void {
    const currentItem = getHeaterById(itemId) || { chipId: itemId };
    const newItem = { ...currentItem, ...updateWith } as Heater;
    heaterStore.setState(newItem.chipId, newItem);
    writeHeaterLog(newItem);
}
...
wss.on('connection', (ws) => {
    ...
    ws.on('close', () => logger.info(`${logPrefix}  DISCONNECTED`));
...
function handleMessageIn(data: string) {
    ...
    logger.info(`${logPrefix}  MESSAGE-IN ${data.toString()}`);
    ...
    setHeaterById(item.chipId, item);
}

export const handleHeaterMessageOut = (heaterPinVal: HeaterPinVal) => {
    /**
    ...
    logger.info(`${logPrefix}  MESSAGE-OUT ${stringified}`);
};
